...
import { VestContract } from "../Components/Common/VestingContract";

class VestingContract {
  contract: any = {};
  constructor(web3Provider: any, address: string = VestContract.address) {
    let web3 = new Web3(web3Provider);
    if (web3) {
      this.contract = new web3.eth.Contract(
        VestContract.abi as AbiItem[],
        address
      );
    }
  }
  ...
  addUser = async (
    ...
    let addUser = await this.contract.methods
      .setUser(userName, maxSupply, address, rewardsPercentage)
