...
class XanaVestingContractServiceV2 {
  contract: any = {};
  constructor(web3Provider: any, address: string = XanaVestingContract_V2.address ) {
    let web3 = new Web3(web3Provider);
    if (web3) {
      this.contract = new web3.eth.Contract(
        XanaVestingContract_V2.abi as AbiItem[],
        address 
      );
    }
  }
  ...
  getClaimableCheck =async(address:string)=>{
    let claimCheck = await this.contract.methods.getDaysClaimable(address).call();
    if(claimCheck) return claimCheck;
    return false;
  }
}

export { XanaVestingContractServiceV2 };
